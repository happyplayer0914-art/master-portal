// =========================================================================
// 4. UI MANAGER & 5. GAME SYSTEM (Ultra Final Restoration)
// =========================================================================
const UIManager = {
    init() { 
        this.initBackground(); 
    ...
    updateIdleUI() {
        ...
        if (display) {
            ...
            display.className = amount >= 100 ? "text-sm font-black text-yellow-400" : "text-sm font-black text-slate-400";
        }
    },
...
const GameSystem = {
    Lobby: {
        ...
        synthesizeItem(id) {
            const item = GameData.items[id]; let count = GameState.inventory.filter(i => i === id).length;
            ...
            const resRarity = tiers[tiers.indexOf(item.rarity) + 1];
            const pool = Object.values(GameData.items).filter(it => it.rarity === resRarity && it.type === item.type);
            const res = pool[Math.floor(Math.random() * pool.length)];
            GameState.inventory.push(res.id); GameState.save(); UIManager.renderInventory(); UIManager.applyAvatarSkin(); UIManager.updateRpgLobbyUI();
            AudioEngine.sfx.gacha_reveal(); UIManager.triggerHeavyHaptic();
            alert(`✨ 합성 대성공!\n\n[${item.name}] 3개가 융합되어...\n🎉 [ ${res.name} ] (이)가 되었습니다!`);
        }
    },
    Gacha: {
        performGacha(times) {
            ...
            if(over) over.classList.add('active'); if(resBox) resBox.classList.add('hidden'); if(anim) anim.classList.remove('hidden');
            ...
            setTimeout(() => {
                ...
                if(anim) anim.classList.add('hidden'); if(resBox) { resBox.classList.remove('hidden'); resBox.innerHTML = ''; }
                ...
                results.forEach((item, index) => {
                    setTimeout(() => {
                        ...
                        if(resBox) resBox.innerHTML += `<div class="gacha-item-card item-card rarity-${item.rarity}"><span class="text-[10px] font-bold mb-1 tracking-widest">[${rarityLabel}]</span><div class="text-4xl mb-1 filter drop-shadow-lg">${item.emoji}</div><h4 class="text-white font-bold text-xs text-center break-keep">${item.name}</h4></div>`;
    ...
    Ranking: {
        ...
        async loadRanking() {
            ...
            } catch(e) { console.error(e); list.innerHTML = '<div class="text-center py-8 text-red-400">불러오기 실패</div>'; }
        }
    },
    Battle: {
        ...
        enterDungeon() {
            ...
            if (!isBoss && parseFloat(stageRoll) < 0.3) { this.triggerRandomEvent(parseFloat(stageRoll)); } 
            else { this.startBattleSequence(isBoss); }
        },
        ...
        endEvent() { GameState.rpgStage++; GameState.save(); const bNav = document.getElementById('bottom-nav'); if(bNav) bNav.style.display = 'flex'; UIManager.navTo('screen-arena', document.querySelectorAll('.nav-item')[1]); },
        startBattleSequence(isBoss) {
            ...
            GameState.isBattling = true; localStorage.setItem('master_in_battle', 'true');
            if (isBoss) {
                AudioEngine.sfx.boss(); const overlay = document.getElementById('boss-warning-overlay'); if(overlay) overlay.classList.add('active');
                let shakes = 0; let shakeInt = setInterval(() => { const appCont = document.querySelector('.app-container'); if(appCont) appCont.classList.add('shake'); UIManager.triggerHeavyHaptic(); setTimeout(() => { if(appCont) appCont.classList.remove('shake'); }, 150); if(++shakes >= 4) clearInterval(shakeInt); }, 500);
                setTimeout(() => { if(overlay) overlay.classList.remove('active'); this.initBattle(true); }, 3000);
            } else { this.initBattle(false); }
        },
        initBattle(isBoss) {
            this.monsterMaxHp = GameState.rpgStage * 40 + (isBoss ? 200 : 0); this.monsterCurrentHp = this.monsterMaxHp;
            ...
            const bCard = document.getElementById('battle-card'); if(bCard) bCard.className = isBoss ? "glass-card battle-card p-6 mb-6 text-center relative border border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.3)]" : "glass-card battle-card p-6 mb-6 text-center relative border border-purple-500/30";
            ...
            clearInterval(this.battleInterval); this.battleInterval = setInterval(() => this.monsterAttack(), 1500); 
        },
        updateBattleUI() {
            const stats = GameState.getTotalStats(); 
        ...
        playerAttack() {
            ...
            const ATTACK_COOLDOWN = 600; if (Date.now() - this.lastAttackTime < ATTACK_COOLDOWN) return;
            this.lastAttackTime = Date.now(); AudioEngine.sfx.hit(); UIManager.triggerHaptic();
            let myAtk = GameState.getTotalStats().atk; let isCrit = Math.random() < 0.2; let damage = isCrit ? Math.floor(myAtk * 1.5) : myAtk;
            this.monsterCurrentHp -= damage;
            const sprite = document.getElementById('monster-sprite'); if(sprite) { sprite.classList.remove('damage-flash'); void sprite.offsetWidth; sprite.classList.add('damage-flash'); }
        ...
        endBattle(isWin) {
            ...
            if (isWin) {
                AudioEngine.sfx.coin(); let rewardGold = (GameState.rpgStage % 5 === 0) ? (GameState.rpgStage * 30) : 10; let rewardGem = (GameState.rpgStage % 5 === 0) ? 50 : 0;
                GameState.gold += rewardGold; GameState.gem += rewardGem; GameState.rpgStage++; GameState.save();
                GameSystem.Ranking.updateRankingSilently(); alert(`🎉 토벌 성공!\n🪙 +${rewardGold}G ${rewardGem > 0 ? '/ 💎 +'+rewardGem : ''}`);
            } else { GameState.currentHp = 0; GameState.save(); alert(`💀 쓰러졌습니다... 휴식이 필요합니다.`); }
